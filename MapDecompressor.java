...
public class MapDecompressor {
	...
	public static byte[] grabMap(int id) throws IOException
	{
		...
		{
			RandomAccessFile raf_cache = new RandomAccessFile(Signlink.findCacheDirectory()+ "maps/MAP_CACHE.dat", "rw");
			RandomAccessFile raf_index = new RandomAccessFile(Signlink.findCacheDirectory()+ "maps/MAP_CACHE.idx", "rw");
	...
	public static void loadIndex2() throws IOException
	{
		...
		DataInputStream dis = new DataInputStream(new FileInputStream(Signlink.findCacheDirectory()+ "maps/MAP_CACHE.idx2"));
		for (int i = 0; i < (int) new File(Signlink.findCacheDirectory()+ "maps/MAP_CACHE.idx2").length() / 2; i++)
...
//					out.write(type);
	...
	public static void spawnObject(int objectId, int x, int y, int z, int rotation, int type) {
			final Class20[] groundData = Class68.aClass20Array1322;
			int localX = x - Class37_Sub18.anInt2200;
			int localY = y - Class15_Sub1.anInt1738;
			int plane = z;
			if(localX > -1 && localY > -1) {
				if ((Class1.aByteArrayArrayArray63[1][localY][localX] & 2) == 2) {
					plane--;
				}
			}
			...
			if(~localY < -1 && ~localX < -1 && ~localY > -104 && ~localX > -104) {
				Class56.method1144(objectId, type, rotation, z, Class38.aClass67_847, false, localY, localX, class20);
			}
	}
